(function($) {
  $.fn.transpose = function(options) {
    ...
    var getChordRoot = function(input) {
      if (input.length > 1 && (input.charAt(1) == 'b' || input.charAt(1) == '#')) { return input.substr(0, 2) } else { return input.substr(0, 1) }
    }
    ...
    var getDelta = function(oldIndex, newIndex) {
      if (oldIndex > newIndex) { return 0 - (oldIndex - newIndex) } else if (oldIndex < newIndex) { return 0 + (newIndex - oldIndex) } else { return 0 }
    }
    ...
    var getNewWhiteSpaceLength = function(a, b, c) {
      if (a > b) { return (c + (a - b)) } else if (a < b) { return (c - (b - a)) } else { return c }
    }
    ...
    return $(this).each(function() {
      ...
      $(keys).each(function(i, key) {
        if (currentKey.name == key.name) { keyLinks.push("<a href='#' class='selected'>" + key.name + '</a>') } else { keyLinks.push("<a href='#'>" + key.name + '</a>') }
      })
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        for (var n = 0; n < videlit.length; n++) {
          if (line.toLowerCase().indexOf(videlit[n]) != -1) {
            line = '<b class="videlit_line">' + line.trim() + '</b>'
          }
        }

        if (isChordLine(line)) { output.push("<span class='chords'>" + wrapChords(line) + '</span>') } else { output.push("<span class='text'>" + line + '</span>') }
      };
